...
/* --------------------------------------------------
 * Архітектурно-залежні макроси та номери системних викликів
 * -------------------------------------------------- */
...
/* --------------------------------------------------
 * Низькорівневі хелпери системних викликів (Syscalls)
 * -------------------------------------------------- */
#if defined(__x86_64__)
...
static long sys_call6(long n, long a, long b, long c, long d, long e, long f) {
    long ret;
    __asm__ volatile ("pushl %%ebp\n\t" "movl %[arg6], %%ebp\n\t" "int $0x80\n\t" "popl %%ebp\n\t" : "=a"(ret) : "0"(n), "b"(a), "c"(b), "d"(c), "S"(d), "D"(e), [arg6] "r"(f) : "memory");
    return ret;
}
...
static long sys_call1(long n, long a) {
    ...
    __asm__ volatile ("ecall" : "=r"(a0) : "r"(a7), "0"(a0) : "memory");
    return a0;
}
static long sys_call2(long n, long a, long b) {
    register long a7 __asm__("a7") = n;
    ...
    __asm__ volatile ("ecall" : "=r"(a0) : "r"(a7), "0"(a0), "r"(a1) : "memory");
    return a0;
}
static long sys_call3(long n, long a, long b, long c) {
    register long a7 __asm__("a7") = n;
...
static long sys_call6(long n, long a, long b, long c, long d, long e, long f) {
    ...
    __asm__ volatile ("ecall" : "=r"(a0) : "r"(a7), "0"(a0), "r"(a1), "r"(a2), "r"(a3), "r"(a4), "r"(a5) : "memory");
    return a0;
}
#endif
...
/* --------------------------------------------------
 * Системні виклики (Адаптовані під кросплатформенність)
 * -------------------------------------------------- */
int close(int fd)
{
...
int socket(int domain, int type, int protocol)
{
    ...
    return (int)sys_call2(SYS_SOCKETCALL, 1, (long)args); /* socketcall(SYS_SOCKET) */
#else
...
int connect(int fd, const struct sockaddr* addr, socklen_t len)
{
    ...
    return (int)sys_call2(SYS_SOCKETCALL, 3, (long)args); /* socketcall(SYS_CONNECT) */
#else
    return (int)sys_call3(SYS_CONNECT, fd, (long)addr, len);
#endif
}
...
void* mmap(void* addr, size_t len, int prot, int flags, int fd, long off)
{
    ...
    long ret = sys_call1(SYS_MMAP, (long)&args);
    return (void*)ret;
#else
    long ret = sys_call6(SYS_MMAP, (long)addr, (long)len, (long)prot, (long)flags, (long)fd, off);
    return (void*)ret;
#endif
}
...
{
    return (int)sys_call2(SYS_MUNMAP, (long)addr, (long)len);
}
...
/* --------------------------------------------------
 * C++ оператори керування пам'яттю
 * -------------------------------------------------- */
#ifdef __cplusplus
